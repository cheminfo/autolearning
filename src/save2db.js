...
if(annotations.length>0&&!toView){
    ...
    if(qresult==null||qresult.length==0){
        var chemical = {fk_userID:0, repository:datasetName};
        //To insert a new entry into the table and returns the autogenerated key if any.
        _chemicalID = db.insert("chemical",chemical);
        qresult = {_moleculeID:sdfi.diaID+"",fk_chemicalID:_chemicalID, file: molFiles[i].catalogID};
    ...
    var atoms = [];
    var atoms = {};
    for (var j=0; j<sdfi.diaIDs.length; j++) {
        //console.log(sdfi.diaIDs[j].id);
        var hosesString=ACT.getHoseCodesFromDiaID(sdfi.diaIDs[j].id, 5,{algorithm:0});


        var atom = {_atomID:sdfi.diaIDs[j].id+"",
            symbol:sdfi.diaIDs[j].element+"",
            canonical_number:sdfi.diaIDs[j].atoms[0],
            hose2:hosesString[1]+"",
            hose3:hosesString[2]+"",
            hose4:hosesString[3]+"",
            hose5:hosesString[4]+"",
            fk_moleculeID:sdfi.diaID+""};
        //Try to insert it. If it already exist do nothin.
        db.insert("atom",atom);
        atoms[sdfi.diaIDs[j].id+""]=atom;

    }

    var spectrum  = {type:"NMR spectrum",
        solvent:solvent+"",
        nuc1:"1H",
        fk_chemicalID:_chemicalID,
        file:molFiles[i].catalogID};

    //Have to include solvent and nucleus in the query. For now, It should work.
    qresult = db.select("SELECT * from spectrum WHERE fk_chemicalID="+_chemicalID+" AND solvent = '"+solvent+"' AND nuc1='1H'");
    var _spectrumID = 0;
    if(qresult==null||qresult.length==0)
    ...
    for(var j=0;j<annotations.length;j++){
        for(var k=0;k<annotations[j]._highlight.length;k++){
            ...
            if(ref_atom){
                ...
                //if(qresult==null||qresult.length==0){
                var assignment = {chemicalShift:(parseFloat(annotations[j].info.stopX)+parseFloat(annotations[j].info.startX))/2,
                    multiplicity:"",
                    integration:1,
                    batchID:iteration,
